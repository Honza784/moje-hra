...
{
	if (hrac_xp >= hrac_xpDoLevelup)
	{
		hrac_xp -= hrac_xpDoLevelup;
		hrac_level++;
		hrac_xpDoLevelup = 20 + hrac_level * 10;
		int bonus = nahodne(1, 3);
...
void nastavPaladina()
{
	...
	schopnost2_nazev = "Uder svetlem"; schopnost2_popis = "1 poskozeni vsem neprum";
...
void nastavMonstrum(int slot, string jmeno, int hp, int atk, int xp, string h1, string h2)
{
	int zlato = 0;
...
void nastavGoblina(int slot)   { nastavMonstrum(slot, "Goblin",    5,  2, 5,  "Goblin vristi: \"Moje zlato! Moje kosti!\"",                         "Goblin vristi: \"Vrazim te do zeme!\""); }
void nastavZombii(int slot)    { nastavMonstrum(slot, "Zombie",    6,  2, 6,  "Zombie mumla: \"Moooozeeek...\"",                                     "Zombie mumla: \"Sluuuuziiiit...\""); }
void nastavTrolla(int slot)    { nastavMonstrum(slot, "Troll",    10,  4, 10, "Troll rve: \"Tamaten hnufak chce nasejc zlato, majznu ho palici!\"", "Troll rve: \"Troll moc hladovy!\""); }
void nastavVlkodlaka(int slot) { nastavMonstrum(slot, "Vlkodlak",  8,  3, 8,  "Vlkodlak vyje: \"Aaauuuu! Krev!\"",                                  "Vlkodlak vrci: \"Moje noci, moje korist!\""); }
void nastavDraka(int slot)     { nastavMonstrum(slot, "Drak",     12,  5, 12, "Drak reve: \"Smrtelnici jsou tak malickati!\"",                       "Drak reve: \"Tvoje kosti ohriji moje vejce!\""); }
void nastavKostlivce(int slot) { nastavMonstrum(slot, "Kostlivec", 6,  3, 7,  "Kostlivec rachoti: \"Klapklapklap!\"",                                "Kostlivec rachoti: \"Nemas sanci!\""); }
void nastavPriseru(int slot)   { nastavMonstrum(slot, "Prisera",   7,  3, 8,  "Prisera vrzci neznamy zvuk.",                                         "Prisera trese vzduchem svymi chapadly."); }
void nastavBanditu(int slot)   { nastavMonstrum(slot, "Bandit",    6,  3, 7,  "Bandit vristi: \"Zlato nebo smrt!\"",                                 "Bandit vristi: \"Mas smulu, pocestny!\""); }

void nastavMiniboss1()
{
	nep1_jmeno = "Zar Stinovy"; nep1_maxZivoty = 22; nep1_zivoty = 22;
	nep1_utok = 6; nep1_zlato = nahodne(8, 15); nep1_xp = 25; nep1_typ = 2;
...
{
	nep1_jmeno = "Krvava Jena"; nep1_maxZivoty = 20; nep1_zivoty = 20;
	nep1_utok = 7; nep1_zlato = nahodne(8, 15); nep1_xp = 30; nep1_typ = 2;
...
bool souboj(bool hracPrvni)
{
	...
	while (hrac_zivoty > 0)
	{
		...
		if (hracNaTahu || kolo == 1)
		{
			zobrazHrace();
			cout << "Nepratele:\n";
			if (nep1_zivoty > 0) cout << "  1) " << nep1_jmeno << " HP:" << nep1_zivoty << "/" << nep1_maxZivoty << "\n";
			...
			cout << "Schopnosti:\n";
			cout << "  1) " << schopnost1_nazev << " - " << schopnost1_popis; if (schopnost1_mana > 0) cout << " [" << schopnost1_mana << " many]"; cout << "\n";
			cout << "  2) " << schopnost2_nazev << " - " << schopnost2_popis; if (schopnost2_mana > 0) cout << " [" << schopnost2_mana << " many]"; cout << "\n";
			cout << "  3) " << schopnost3_nazev << " - " << schopnost3_popis; if (schopnost3_mana > 0) cout << " [" << schopnost3_mana << " many]"; cout << "\n";
			...
			if (s_heal > 0 && s_posk == 0) { hrac_zivoty += s_heal; if (hrac_zivoty > hrac_maxZivoty) hrac_zivoty = hrac_maxZivoty; cout << "Obnovil sis " << s_heal << " HP.\n"; }
		...
		if (nep1_zivoty > 0)
		{
			...
			{
				if (sance(50)) cout << "\n" << nep1_hlaska1 << "\n"; else cout << "\n" << nep1_hlaska2 << "\n";
				hrac_zivoty -= nep1_utok; if (hrac_zivoty < 0) hrac_zivoty = 0;
...
void vesnice(int cislo)
{
	...
	{
		cout << "\nCo chces udelat?\n";
		cout << "1) Doplnit zivoty (2 zlata/HP)\n";
		cout << "2) Zvysit max HP o 2 (10 zlata)\n";
		cout << "3) Zvysit max manu o 2 (8 zlata)\n";
		cout << "4) Zvysit utok o 1 (12 zlata)\n";
		cout << "5) Odejit\nVolba: ";
		...
		if (v == 1)
		{
			...
			else if (hrac_zlato < 2) cout << "Nemas dost zlata.\n";
			else
			{
		...
		else if (v == 2) { if (hrac_zlato < 10) cout << "Nemas dost zlata (10).\n"; else { hrac_zlato -= 10; hrac_maxZivoty += 2; hrac_zivoty += 2; cout << "Max HP: " << hrac_maxZivoty << "\n"; } }
		else if (v == 3) { if (hrac_zlato < 8)  cout << "Nemas dost zlata (8).\n";  else { hrac_zlato -= 8;  hrac_maxMana  += 2; hrac_mana  += 2; cout << "Max mana: " << hrac_maxMana  << "\n"; } }
		else if (v == 4) { if (hrac_zlato < 12) cout << "Nemas dost zlata (12).\n"; else { hrac_zlato -= 12; hrac_utok++; schopnost1_poskozeni = hrac_utok; cout << "Utok: " << hrac_utok << "\n"; } }
		else if (v == 5) { cout << "Odchazis.\n"; konec = true; }
		else cout << "Neplatna volba.\n";
	}
}
...
void moudryStrom()
{
	...
	cout << "Strom: \"Zdravim! Odvazujes se pomerit sily mysli?\"\n";
	cout << "1) Ano  2) Ne\nVolba: ";
	int v; cin >> v; cin.ignore(10000, '\n');
	if (v != 1) { cout << "Strom: \"Jak chces...\"\n"; return; }

	cout << "Strom: \"Kazdy den na obloze se vznasi, radost nam prinasi.\n";
	cout << "  Bubaky strasi. Co to je?\"\n";
	cout << "Odpoved: ";
	...
	if (ok) { cout << "Strom: \"Spravne! +2 max zivoty.\"\n"; hrac_maxZivoty += 2; hrac_zivoty += 2; if (hrac_zivoty > hrac_maxZivoty) hrac_zivoty = hrac_maxZivoty; }
	else    { cout << "Strom: \"Spatne! -2 zlata.\"\n"; hrac_zlato -= 2; if (hrac_zlato < 0) hrac_zlato = 0; }
	enter();
}
...
int main()
{
	...
	if (!prohra) { cout << "\nPri ceste te prepadl Goblin!\n";            nastavGoblina(1);                         pocetNepriatel = 1; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\nZe tmy vylezla Zombie!\n";                  nastavZombii(1);                          pocetNepriatel = 1; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\nGoblin a Bandit vam zahrazuji cestu!\n";    nastavGoblina(1); nastavBanditu(2);       pocetNepriatel = 2; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\n=== ZAR STINOVY blokuje branu! ===\n";      nastavMiniboss1();                                           if (!souboj(false)) prohra = true; }
	if (!prohra) { vesnice(2); }
	if (!prohra) { cout << "\nObri Troll se vali z lesa!\n";              nastavTrolla(1);                          pocetNepriatel = 1; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\nVlkodlak a Kostlivec vas napadaji!\n";      nastavVlkodlaka(1); nastavKostlivce(2);  pocetNepriatel = 2; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\nMlady Drak pristava pred tebou!\n";         nastavDraka(1);                           pocetNepriatel = 1; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\n=== KRVAVA JENA blokuje most! ===\n";       nastavMiniboss2();                                           if (!souboj(false)) prohra = true; }
	if (!prohra) { moudryStrom(); }
	if (!prohra) { cout << "\nDve Prisery vas obklicuji!\n";              nastavPriseru(1); nastavZombii(2);        pocetNepriatel = 2; if (!souboj(true))  prohra = true; }
	if (!prohra) { cout << "\nTri nepratele na skalach!\n";               nastavGoblina(1); nastavTrolla(2); nastavBanditu(3); pocetNepriatel = 3; if (!souboj(true)) prohra = true; }
	if (!prohra) { vesnice(3); }
